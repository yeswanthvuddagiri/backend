...
function validateConflicts(options, connectionString) {
    ...
    if (options.gssapiServiceName &&
        (connectionString === null || connectionString === void 0 ? void 0 : connectionString.searchParams.has('authMechanismProperties'))) {
...
function validateHostSeedList(hosts, fixedPort) {
    ...
    for (const h of hosts) {
        const { host, port } = (_b = (_a = /^(?<host>.+?)(:(?<port>\d+))?$/.exec(h)) === null || _a === void 0 ? void 0 : _a.groups) !== null && _b !== void 0 ? _b : {};
...
function generateUriNormalized(options) {
    ...
    if (replSetName || (hosts !== null && hosts !== void 0 ? hosts : []).length > 1) {
        const connectionString = new mongodb_connection_string_url_1.ConnectionString(`mongodb://replacemeHost/${encodeURIComponent(uri || '')}`);
        connectionString.hosts = validateHostSeedList(hosts !== null && hosts !== void 0 ? hosts : [], options.port);
        if (replSetName)
